...
def plotOutput(forwardconf,inverseconf,mastereps,masterts,tslength,forwardtitle,inversetitle,logs = [1,1]):
...
if __name__ == '__main__':
    ...
    mastereps=np.array([0.005,0.0075,0.01,0.0125,0.015,0.02,0.04]) #for z and w
    mastereps=np.array([0.00001,0.00005,0.0001,0.0005,0.001,0.005,0.0075]) #for x and y
    # mastereps=np.array([0.02,0.05,0.1,0.15,0.2,0.25,0.3,0.35,0.4]) #for x and w
    ...
    # PS.testDoublePendulumModified([0,1],masterts,mastereps,fname = basedir+fname,lags = [[100,100]])
    outdict = fileops.loadPickle(basedir+fname)
    logs = [0,0]
    print(outdict['mastereps'])
    print(outdict['forwardconf'])
    print(outdict['inverseconf'])
    plotOutput(outdict['forwardconf'],outdict['inverseconf'],outdict['mastereps'],outdict['masterts'],len(outdict['ts']),outdict['forwardtitle'],outdict['inversetitle'],logs)
