...
def plotOutput(contconf,invcontconf,Nlist,title,mastereps=np.array([0.005,0.01,0.02,0.05,0.1,0.2])):
    plt.figure(0)
    plt.plot(mastereps,contconf)
    plt.legend([str(N) for N in Nlist])
    plt.ylabel(r'\theta (C^0)')
    plt.xlabel(r'\epsilon')
    plt.title(title+' forward')
    plt.figure(1)
    plt.plot(mastereps,invcontconf)
    plt.legend([str(N) for N in Nlist])
...
def testLorenz(Nlistmaster):
    ...
    contconf, invcontconf = PM.convergenceWithContinuityTest(Mx,My,Nlist)
    title = eqns + 'M{0} -> M{1}'.format(compind1,compind2)
    plotOutput(contconf,invcontconf,Nlist,title)

if __name__ == '__main__':
    Nlistmaster = np.arange(0.1,0.6,0.1)
    testLorenz(Nlistmaster)